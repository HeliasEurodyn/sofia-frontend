...
node
{
    try{
        ...
        {
            git branch: 'master',
            url: 'https://{token}@github.com/HeliasEurodyn/sofia-frontend.git'
        }
        stage("Build")
        {
            nodejs('NodeJs') {
        ...
        stage("Deploy")
        {
            withCredentials([usernamePassword(credentialsId: 'onenet', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')])
            {
                ...
                sshCommand remote: remote, command: "cd /home/engonenet/ed/onenet_for_jenkins/frontend-container;docker-compose down"
                sshRemove remote: remote, path: '/home/engonenet/ed/onenet_for_jenkins/frontend-container/sofia-frontend/dist'
                sshPut remote: remote, from: './dist', into: '/home/engonenet/ed/onenet_for_jenkins/frontend-container/sofia-frontend/'
                sshCommand remote: remote, command: "cd /home/engonenet/ed/onenet_for_jenkins/frontend-container;docker-compose up --build -d"
            }
        }
